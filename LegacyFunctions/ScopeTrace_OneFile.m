...
%
    ...
    methods
        function obj = ScopeTrace(args)
            ...
            arguments
                ...
                args.Echo          logical = false;
                args.CachedTrace   logical = false;
            end
            
            if ~isfield(args, "FilePath")
                [file, path] = uigetfile("*");
                args.FilePath = fullfile(path, file);
                clearvars file path
            end
            
            obj.FilePath    = args.FilePath;
            obj.Echo        = args.Echo;
            obj.CachedTrace = args.CachedTrace;
        ...
        function Channel            = get.Channel(obj)
            ...
            switch obj.TraceType
                case "Tektronix (.isf)"
                    ...
                    Channel = Channel{1};
                case "LeCroy (.trc)"
                    Channel = obj.Info.wave_source;
                    if Channel.startsWith("C")
                        Channel = Channel.insertAfter("C","h");
                    else
                        Channel = "???";
                    end
                otherwise
    ...
    methods (Access = private)
        function obj                = GetTraceType(obj)
            switch obj.FilePath.extractAfter(".")
                case "trc"
        ...
        function obj                = GetLeCroyTrcInfo(obj)
            ...
            if obj.Info.timebase == 100
                ...
                tmp=["1 ps / div";"2 ps / div";"5 ps / div";"10 ps / div";"20 ps / div";"50 ps / div";"100 ps / div";"200 ps / div";"500 ps / div";
            ...
            tmp=["1 uV / div";"2 uV / div";"5 uV / div";"10 uV / div";"20 uV / div";"50 uV / div";"100 uV / div";"200 uV / div";"500 uV / div";
...
%                 RisTimeArray.ris_offset = fread(obj.fid,obj.Info.ris_sweeps,"float64");
        ...
        function obj                = GetLeCroyDatInfo(obj)
            keyboard
            CompressedFilePath = obj.FilePath.extractBefore(".") + "CompressedTime";
        ...
        function Voltage            = GetLeCroyDatVoltage(obj)
            ...
            CompressedFilePath = obj.FilePath.extractBefore(".") + "CompressedVoltage";
    ...
    methods (Access = private, Static)
        ...
        % Compressed Waveform Storage (for LeCroy (.dat) Tektonix (.dat) and SimpleCSV (.Simplecsv)
        ...
        function s = ReadString(        fid, Addr)
            ...
            s = string(deblank(fgets(fid, 16))); %read the next 16 characters of the line (all strings in lecroy binary file are 16 characters long)
        end
        ...
        function s = ReadUnitDefinition(fid, Addr)
            fseek(fid, Addr, "bof"); %move to the address listed in relation to the beginning of the file
            s = string(deblank(fgets(fid, 48))); %read the next 48 characters (all strings in lecroy .trc file are 16 characters long)
        end
